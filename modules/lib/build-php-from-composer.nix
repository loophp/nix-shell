...
let
  getExtensionsFromSection =
    ...
    let
        ...
        let
          filecontent =
            if builtins.pathExists composerJson then
              builtins.fromJSON (builtins.readFile composerJson)
            else
              { };
        in
      ...
      ) composerRequiresKeys;
    in
    # Convert the set into a list, filter out values not starting with "ext-", get rid of the first 4 characters from the name
    map (x: builtins.substring 4 (builtins.stringLength x) x) (
      builtins.filter (x: (builtins.substring 0 4 x) == "ext-") (lib.flatten composerRequiresMap)
    );
in
{
  perSystem = {
    lib.buildPhpFromComposer =
      {
        src,
        php ? inputs.php,
        extensions ? [ ],
      ...
      (phpDrv.override flags).buildEnv {
        extraConfig = lib.concatStrings [
          ...
          (lib.optionalString (builtins.pathExists "${builtins.getEnv "PWD"}/.user.ini") (
            builtins.readFile "${builtins.getEnv "PWD"}/.user.ini"
          ))
        ];
        ...
        extensions =
          ...
          let
            buildExtensions =
              ...
              let
                filterStringExtensions =
                  extList:
                  builtins.filter (
                    ext:
                    (builtins.isString ext)
                    && (lib.warnIf (!(all ? "${ext}"))
                      "The ${ext} extension does not exist or is enabled by default in PHP ${phpDrv.version}, ignoring."
                      (all ? "${ext}")
                    )
                  ) extList;
                ...
                e1 = builtins.filter (ext: (builtins.tryEval all."${ext}".outPath).success) e0;
                ...
                e5 = builtins.filter (ext: !builtins.elem ext (filterDrvExtensions withoutExtensions)) e4;
              in
              e5;
          in
          buildExtensions {
